  ...
  <div class="recycle-tab-wrapper">
    ...
    <bk-dialog
      ...
      <div class="delete-content">
        <p>{{ $t('该操作会彻底清理当前组织数据，包括组织下的用户数据及其组织关系，永久无法恢复，请谨慎操作。') }}</p>
    ...
    <bk-sideslider
      ...
      <div slot="content">
        <!-- 批量预览 -->
        <DepartmentBatchReduction
          v-if="isDepartmentBatchReduction"
...
<script>
import DepartmentBatchReduction from './DepartmentBatchReduction.vue';
...
export default {
  ...
  data() {
    return {
      deleteDialog: {
        ...
        width: 480,
        headerPosition: 'left',
        title: this.$t('确认要永久删除当前组织？'),
      },
      reductionSetting: {
        isShow: false,
        title: this.$t('组织还原预览'),
      },
      isDepartmentBatchReduction: false,
      isDepartmentReduction: false,
      departmentMap: [{
        id: 'department_name',
        label: this.$t('组织名'),
        disabled: true,
  ...
  methods: {
    ...
    handleBatchReduction() {
      this.isDepartmentBatchReduction = true;
      this.reductionSetting.isShow = true;
    },
    // 还原
    handleReduction(row) {
      this.selectedList = [row];
      this.isDepartmentReduction = true;
      this.reductionSetting.isShow = true;
    },
  },
};
