...
from bkuser.apps.idp.models import IdpPlugin
from bkuser.idp_plugins.constants import CUSTOM_PLUGIN_ID_PREFIX, MAX_LOGO_SIZE
...
class Command(BaseCommand):
    ...
    def handle(self, dir_name, *args, **options):
        plugin_base_dir = settings.BASE_DIR / "bkuser" / "idp_plugins"
        # 1. 检查指定的插件目录是否存在
        if not os.path.isdir(plugin_base_dir / dir_name):
            raise RuntimeError(f"plugin directory [{dir_name}] not found in bkuser/idp_plugins!")

        # 2. 检查自定义插件是否配置 Metadata
        try:
            metadata = import_string(f"bkuser.idp_plugins.{dir_name}.METADATA")
        except ImportError:
            raise RuntimeError("custom idp plugin must set metadata!")

        # 3. 确保自定义插件的 ID 符合规范
        if not metadata.id.startswith(CUSTOM_PLUGIN_ID_PREFIX):
            raise RuntimeError(f"custom idp plugin's id must start with `{CUSTOM_PLUGIN_ID_PREFIX}`")
        ...
        # 6. 如果同名插件已存在，更新，否则创建
        IdpPlugin.objects.update_or_create(
            id=metadata.id,
            defaults={
                "name": metadata.name,
        ...
        # 7. 注册到 DB 成功要给提示
        self.stdout.write(f"register idp plugin [{metadata.id}] into database successfully.")
