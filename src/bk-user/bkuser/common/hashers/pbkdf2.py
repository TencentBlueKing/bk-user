...
def _pbkdf2_hmac_sm3(password: bytes, salt: bytes, iterations: int, dk_len: int | None = None):
    """
...
class PBKDF2SM3PasswordHasher(BasePasswordHasher):
    ...
    def encode(self, password: str, salt: str | None = None, iterations: int | None = None):
        """根据指定的原始密码，盐值，迭代次数生成加密信息"""
        salt = salt or self.salt()
        iterations = iterations or self.iterations
        hash = _pbkdf2_hmac_sm3(force_bytes(password), force_bytes(salt), iterations)
    ...
    def must_update(self, encoded: str):
        """根据盐长度是否符合预期，以及当前加密信息的迭代次数，判断是否增加迭代次数（空转）"""
        decoded = self.decode(encoded)
        update_salt = must_update_salt(decoded["salt"], self.salt_entropy)
        return (decoded["iterations"] != self.iterations) or update_salt

    def harden_runtime(self, password: str, encoded: str):
