...
class TenantUserRetrieveUpdateDestroyApi(
    ...
    def delete(self, request, *args, **kwargs):
        ...
        add_audit_record(
            operator=request.user.username,
            tenant_id=self.get_current_tenant_id(),
            operation=OperationEnum.DELETE_USER,
            object_type=ObjectTypeEnum.USER,
...
class TenantUserStatusUpdateApi(CurrentUserTenantMixin, ExcludePatchAPIViewMixin, generics.UpdateAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        # 【审计】记录变更前数据
        data_before = {"status": tenant_user.status}
        ...
        objects = [
            AuditObject(
                ...
                extras={
                    ...
                    "extras": user.extras,
                    "department": tenant_dept.data_source_department_id,
                },
            )
            for user in data_source_users
        ]
...
class TenantUserLeaderBatchUpdateApi(
    ...
    def put(self, request, *args, **kwargs):
        ...
        batch_add_audit_records(
            operator=request.user.username,
            tenant_id=cur_tenant_id,
            operation=OperationEnum.MODIFY_USER_LEADERS,
            object_type=ObjectTypeEnum.USER,
            objects=[
                AuditObject(
                    id=user.id,
