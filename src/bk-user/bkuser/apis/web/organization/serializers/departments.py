...
from bkuser.biz.validators import validate_local_real_data_source_id
...
def _validate_duplicate_dept_name_in_brothers(
    ...
    name: str,
    data_source_id: int,
    exclude_department: DataSourceDepartment | None = None,
) -> None:
    # 获取兄弟部门的数据源部门 ID，排除自己
    # Q: 为什么不使用 parent_dept_relation.get_children
    # A: 如果是根部门，parent_dept_relation 为 None，会报错 :)
    qs = DataSourceDepartmentRelation.objects.filter(parent=parent_relation, data_source_id=data_source_id)
    if exclude_department:
        qs = qs.exclude(department=exclude_department)
    brother_data_source_dept_ids = qs.values_list("department_id", flat=True)
...
class TenantDepartmentCreateInputSLZ(serializers.Serializer):
    ...
    def validate(self, attrs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if parent_tenant_dept_id := attrs["parent_department_id"]:
            parent_tenant_dept = TenantDepartment.objects.get(id=parent_tenant_dept_id, tenant_id=tenant_id)
            # 一致性校验：父部门必须属于所指定的数据源，防止跨源
            if parent_tenant_dept.data_source_id != attrs["data_source_id"]:
                raise ValidationError(_("父部门不属于当前的数据源"))
            parent_dept_relation = DataSourceDepartmentRelation.objects.get(
                department=parent_tenant_dept.data_source_department, data_source_id=attrs["data_source_id"]
            )

        _validate_duplicate_dept_name_in_brothers(parent_dept_relation, dept_name, attrs["data_source_id"])
...
class TenantDepartmentUpdateInputSLZ(serializers.Serializer):
    ...
    def validate_name(self, name: str) -> str:
        ...
        # 在兄弟部门中检查是否有同名的
        _validate_duplicate_dept_name_in_brothers(
            parent_dept_relation, name, data_source_dept.data_source_id, exclude_department=data_source_dept
        )
...
class TenantDepartmentParentUpdateInputSLZ(serializers.Serializer):
    ...
    def validate_parent_department_id(self, parent_dept_id: int) -> int:
        ...
        _validate_duplicate_dept_name_in_brothers(
            ...
            data_source_dept.name,
            data_source_dept.data_source_id,
            exclude_department=data_source_dept,
        )
