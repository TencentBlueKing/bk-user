...
class TenantUserUpdateDestroyAuditor:
    ...
    def pre_record_data_before(
        ...
        # 记录修改前的用户部门
        self.data_befores[tenant_user_id]["department_ids"] = list(
    ...
    def batch_pre_record_data_before(
        ...
        for tenant_user in tenant_users:
            ...
            self.pre_record_data_before(tenant_user, data_source_user)
    ...
    def get_current_operation(self, operation: OperationEnum) -> OperationEnum:
        ...
        operation_map = {
            ...
            OperationEnum.MODIFY_USER_DEPARTMENT_RELATIONS: OperationEnum.DELETE_USER_DEPARTMENT_RELATIONS,
            OperationEnum.MODIFY_USER_LEADER_RELATIONS: OperationEnum.DELETE_USER_LEADER_RELATIONS,
    ...
    def record(self, tenant_user: TenantUser, data_source_user: DataSourceUser):
        ...
        ds_user_id = (
            self.data_befores[tenant_user.id]["data_source_user"]["id"] if self.is_delete else data_source_user.id
        )
        ds_user_name = (
            self.data_befores[tenant_user.id]["data_source_user"]["username"]
            if self.is_delete
        ...
        self.audit_objects = [
            ...
            AuditObject(
                **ds_user_object,
                ...
                data_before=self.data_befores[tenant_user.id]["data_source_user"],
                data_after=get_model_dict(data_source_user) if not self.is_delete else {},
            ),
            # 数据源用户的部门
            AuditObject(
                **ds_user_object,
                operation=self.get_current_operation(OperationEnum.MODIFY_USER_DEPARTMENT_RELATIONS),
                data_before={"department_ids": self.data_befores[tenant_user.id]["department_ids"]},
                data_after={
                    "department_ids": list(
                        DataSourceDepartmentUserRelation.objects.filter(
                            user=data_source_user,
            ...
            AuditObject(
                **ds_user_object,
                operation=self.get_current_operation(OperationEnum.MODIFY_USER_LEADER_RELATIONS),
                data_before={"leader_ids": self.data_befores[tenant_user.id]["leader_ids"]},
                data_after={
                    "leader_ids": list(
            ...
            AuditObject(
                ...
                data_before=self.data_befores[tenant_user.id]["tenant_user"],
        ...
        if self.is_delete:
            self.audit_objects.extend(
                [
                    ...
                    for user_id, user_data in self.data_befores[tenant_user.id]["collaboration_tenant_users"].items()
                ]
            )
