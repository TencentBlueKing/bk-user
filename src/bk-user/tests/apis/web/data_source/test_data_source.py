...
class TestDataSourceDestroyApi:
    ...
    def test_destroy_with_reset_idp_config(self, api_client, data_source, local_idp, wecom_idp, invalid_idp):
        resp = api_client.delete(
            reverse("data_source.retrieve_update_destroy", kwargs={"id": data_source.id}),
        ...
        assert not Idp.objects.filter(id=invalid_idp.id).exists()
        assert not IdpSensitiveInfo.objects.filter(idp_id=wecom_idp.id).exists()
        assert not IdpSensitiveInfo.objects.filter(idp_id=invalid_idp.id).exists()
