...
def flatten(nested_list, types=(list, float)):
    """
    Flat nested list of lists into a single list.
    """
    single_list = [item if isinstance(item, types) else [item] for item in nested_list]
    return [item for sublist in single_list for item in sublist]
