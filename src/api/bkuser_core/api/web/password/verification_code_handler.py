...
class ResetPasswordVerificationCodeHandler:
    ...
    def generate_and_send_verification_code(self, profile_id) -> None:
        self.profile = Profile.objects.get(id=profile_id)
        self.config_loader = ConfigProvider(category_id=self.profile.category_id)

        cache_key = str(self.profile.id)

        self._check_repeat_send_require(cache_key)
        ...
        logger.info(
            "Set the verification_code_data in redis. profile<%s-%s>",
            self.profile.id,
            f"{self.profile.username}@{self.profile.domain}",
        )
        self._set_into_cache(cache_key, json.dumps(verification_code_data), expire_seconds, prefix="reset_password")
    ...
    def verify_verification_code(self, profile_id: int, verification_code: str) -> int:
        ...
        verification_code_data_bytes = self._get_from_cache(cache_key, prefix="reset_password")

        if not verification_code_data_bytes:
            logger.info(
                "verify verification_code, verification_code_data is invalid. profile_id=%s",
                profile_id,
            )
        ...
        logger.info(
            "verify verification_code. profile_id=%s",
            profile_id,
        )
        ...
        if verification_code_data["verification_code"] != verification_code:
            ...
            self._set_into_cache(
                cache_key, json.dumps(verification_code_data), expired_second, prefix="reset_password"
            )
        ...
        self._delete_from_cache(cache_key, prefix="reset_password")
        return profile.id
