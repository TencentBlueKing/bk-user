...
- 提交标记及其含义以 `docs/contributing.md` 的“GIT提交规范”为唯一事实来源，不在本文件重复维护完整标记表。
- commit message 格式为 `<标记>: <中文概要> [TAPD/Issue 关键字]`。标题概要和正文必须使用中文，标记与平台关键字保留规范原文；一个提交只选择最符合主要改动性质的标记。
